...
<body class="gray-bg">
        ...
        <div class="row">
			  <div class="col-lg-2">
				<div class="input-group">
			      ...
			      <input type="text" name="sdName" class="form-control" id="txt_search_sdName" >
				</div>
			  </div>
			  <div class="col-lg-2">
				<div class="input-group">
			      <span class="input-group-addon">生日 </span>
			      <input type="date" name="sdbirth1" class="" id="txt_search_sdbirth1" >—
			      <input type="date" name="sdbirth2" class="" id="txt_search_sdbirth2">
				</div>
			  </div>
			  <div class="col-lg-2">
				<div class="input-group">
					...
					<form id="form_user" method="post" action="reserveUser.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								...
								<td>学生性别：</td>
								<td>
								<input type="radio" id="sdsex1" name="sdsex" value="男"
									class="" aria-required="true" required/>男
								<input type="radio" id="sdsex2" name="sdsex" value="女"
							...
							<tr>
								<td>爱好：</td>
								<td colspan="4">
									<input type="checkbox" id="sdHobby1" name="sdHobby" value="篮球"
									...
									class="" aria-required="true" required/>排球
								</td>
							</tr>
							<tr>
								<td>生日：</td>
								<td colspan="4">
									<input type="date" id="sdbirth" name="sdbirth" 
	...
	<!-- 新增和修改对话框 -->
	<div class="modal fade" id="modal_m_edit" role="dialog" aria-labelledby="modal_user_edit" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_m" method="post" action="reserveM.htm">
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>专业名称：</td>
						...
						<div class="modal-footer">
							<button type="button" class="btn btn-primary"  id="submit_form_m_btn">保存</button>
							<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
						</div>
					</form>
	...
	<div class="modal fade" id="modal_user_count" role="dialog" aria-labelledby="modal_user_del" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-header">
					 <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					 <h4 class="modal-title" id="modal_user_del_head"> 专业人数统计  </h4>
				</div>
				<div class="modal-body">
	...
	$(function () {
	    ...
	    $("#submit_form_m_btn").click(function(){
	    	$("#form_m").submit();
	    });
	});
	...
	var TableInit = function () {
	    ...
	    oTableInit.queryParams = function (params) {
	        var temp = {//这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
	            ...
	            sdName: $("#txt_search_sdName").val(),
	            sdbirth1: $("#txt_search_sdbirth1").val(),
	            sdbirth2: $("#txt_search_sdbirth2").val(),
	...
	var ButtonInit = function () {
	    ...
	    oInit.Init = function () {
	        ...
	        $("#btn_addm").click(function(){
	    		$("#form_m").resetForm();
	    		$('#modal_m_edit').modal({backdrop: 'static', keyboard: false});
				$('#modal_m_edit').modal('show');
	        });
	...
	function initEditUser(getSelection){
		...
		var sdsex = getSelection.sdsex;
		if(sdsex=="男"){
			$('#sdsex1').prop("checked",true);
		}else{
			$('#sdsex2').prop("checked",true);
		}
		
		
		var sdHobby = getSelection.sdHobby;
		if(sdHobby.indexOf("篮球")!=-1){
			$('#sdHobby1').prop("checked",true);
		}else{
			$('#sdHobby1').prop("checked",false);
		}
		if(sdHobby.indexOf("足球")!=-1){
			$('#sdHobby2').prop("checked",true);
		}else{
			$('#sdHobby2').prop("checked",false);
		}
		if(sdHobby.indexOf("排球")!=-1){
			$('#sdHobby3').prop("checked",true);
		}else{
			$('#sdHobby3').prop("checked",false);
		}
